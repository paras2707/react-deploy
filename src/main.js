import React from 'react'
import Zebpay from './images/zebpay.png'
import bit from './images/bit.png'
import banner from './images/banner.PNG'
import banner2 from './images/banner2.PNG'

function Main(props) {
    return (
        <main>
            <h3 className="heading">Best Price to Trade</h3>
            <div className="numbers">
                <div className="mins">
                    <h3>0.31 %</h3>
                    <p>5 Mins</p>
                </div>
                <div className="hours">
                    <h3>0.73 %</h3>
                    <p>1 Hour</p>
                </div>
                <div className="price">
                    <h3>₹ 39,93,493</h3>
                    <p>Average BTC/INR net price including commission</p>
                </div>
                <div className="day">
                    <h3>8.82 %</h3>
                    <p>1 Day</p>
                </div>
                <div className="seven-days">
                    <h3>9.75 %</h3>
                    <p>7 Days</p>
                </div>
            </div>
            <div className="banner">
                <img src={banner} alt="" />
            </div>
            <section className="table">
                <div style={{ overflowX: 'auto' }}>
                    <table>
                        <thead>
                            <tr>
                                <th>
                                    <h4 className="pointer">#</h4>
                                </th>
                                <th>
                                    <h4 className="pointer">Platform</h4>
                                </th>
                                <th>
                                    <h4 className="pointer">Last Traded Price</h4>
                                </th>
                                <th>
                                    <h4 className="pointer">Buy / Sell Price</h4>
                                </th>
                                <th>
                                    <h4 className="pointer">Difference</h4>
                                </th>
                                <th>
                                    <h4 className="pointer">Savings</h4>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                <td>
                                    <h4>1</h4>
                                </td>
                                <td>
                                    <a href="https://wazirx.com/invite/sp7pvbt6?utm_source=finstreet&utm_medium=affiliate&utm_campaign=regnow-btn">
                                        <h4><img className="exchange-logo" src="data:image/png;base64," alt="" /><span className="exchange-name">WazirX</span></h4>
                                    </a>
                                </td>
                                <td>
                                    <h4>₹ {props.item.last}</h4>
                                </td>
                                <td>
                                    <h4>₹ {props.item.buy} / ₹ {props.item.sell} </h4>
                                </td>
                                <td>
                                    <h4 className="blue">{props.diff} %</h4>
                                </td>
                                <td>
                                    <h4 className="blue">▲ ₹ {props.saving}</h4>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <h4>2</h4>
                                </td>
                                <td>
                                    <a href="https://bit.ly/2BJxccc">
                                        <h4><img className="exchange-logo" src={bit} alt="" /><span className="exchange-name">Bitbns</span></h4>
                                    </a>
                                </td>
                                <td>
                                    <h4>₹ 43,51,275</h4>
                                </td>
                                <td>
                                    <h4>₹ 43,51,267 / ₹ 43,54,186 </h4>
                                </td>
                                <td>
                                    <h4 className="red">-0.35 %</h4>
                                </td>
                                <td>
                                    <h4 className="red">▼ ₹ 15,403</h4>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <h4>3</h4>
                                </td>
                                <td>
                                    <a href=".">
                                        <h4><img className="exchange-logo" src="data:image/png;base64," alt="" /><span className="exchange-name">Giotus</span></h4>
                                    </a>
                                </td>
                                <td>
                                    <h4>₹ 43,60,000</h4>
                                </td>
                                <td>
                                    <h4>₹ 43,44,104 / ₹ 43,96,190 </h4>
                                </td>
                                <td>
                                    <h4 className="red">-0.15 %</h4>
                                </td>
                                <td>
                                    <h4 className="red">▼ ₹ 6,678</h4>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <h4>4</h4>
                                </td>
                                <td>
                                    <a href=".">
                                        <h4><img className="exchange-logo" src="data:image/png;base64," alt="" /><span className="exchange-name">Colodax</span></h4>
                                    </a>
                                </td>
                                <td>
                                    <h4>₹ 43,53,883</h4>
                                </td>
                                <td>
                                    <h4>₹ 43,45,555 / ₹ 43,89,230 </h4>
                                </td>
                                <td>
                                    <h4 className="red">-0.29 %</h4>
                                </td>
                                <td>
                                    <h4 className="red">▼ ₹ 12,795</h4>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <h4>5</h4>
                                </td>
                                <td>
                                    <a href=".">
                                        <h4><img className="exchange-logo" src={Zebpay} alt="" /><span className="exchange-name">Zebpay</span></h4>
                                    </a>
                                </td>
                                <td>
                                    <h4>₹ 43,94,000</h4>
                                </td>
                                <td>
                                    <h4>₹ 43,94,000 / ₹ 43,91,009 </h4>
                                </td>
                                <td>
                                    <h4 className="blue">0.63 %</h4>
                                </td>
                                <td>
                                    <h4 className="blue">▲ ₹ 27,321</h4>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <h4>6</h4>
                                </td>
                                <td>
                                    <a href=".">
                                        <h4><img className="exchange-logo" src="data:image/png;base64," alt="" /><span className="exchange-name">CoinDCX</span></h4>
                                    </a>
                                </td>
                                <td>
                                    <h4>₹ 43,96,715</h4>
                                </td>
                                <td>
                                    <h4>₹ 43,84,527 / ₹ 43,96,715 </h4>
                                </td>
                                <td>
                                    <h4 className="blue">0.69 %</h4>
                                </td>
                                <td>
                                    <h4 className="blue">▲ ₹ 30,036</h4>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </section>
            <div className="banner banner2">
                <img src={banner2} alt="" />
            </div>
        </main>

    )
}

export default Main